...
from vie_quotidienne.models import Hebergement, TypeHebergement
from vie_quotidienne.serializers import HebergementSerializer, HebergementDetailSerializer
...
class HebergementViewsetTest(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        heberg_user = User.objects.create_user(username='testHebergeur', password="issou1234#")
        group = Group.objects.create(name="Hebergeur")
        heberg_user.groups.add(group)
        heberg_user.save()
        TypeHebergement.objects.create(libelle="test")
        Hebergement.objects.create(nom="test", description="meh", adresse="meh", owner=superuser,
                                   type=TypeHebergement.objects.first())

    def setUp(self):
        self.root, self.user, self.heberg_user = User.objects.all()
        self.client = APIClient()

        self.viewset_name = 'hebergement'
        self.serializer = HebergementSerializer
        self.detail_serializer = HebergementDetailSerializer
        self.test_object = Hebergement
        self.data = {
            "nom": "test2",
            "adresse": "meh",
            "description": "blu",
            "type": TypeHebergement.objects.first().id,
            "owner": User.objects.get(username='testHebergeur').id
        }
    ...
    def test_endpoint_create(self):
        self.assertTrue(self.client.login(username='testHebergeur', password='issou1234#'))
        url = reverse(self.viewset_name + '-list')
